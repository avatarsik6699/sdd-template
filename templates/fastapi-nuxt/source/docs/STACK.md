...
> **`uv` is required for `sdd init` and `./scripts/init-project.sh`.** The compatibility init script regenerates `uv.lock`
...
After running `uv run sdd init --template fastapi-nuxt --project-name <slug> <target-dir>` and then `./scripts/init-project.sh` inside the generated copy:
...
`sdd init` is the canonical project-copy entrypoint in this slice. `init-project.sh` remains the compatibility step that creates `.env`, injects fresh secrets, and rewrites template placeholders for the current reference stack.
...
gate dispatch lives in `.sdd/template-manifest.yaml`, which points at this helper/docs pair. If
the stack changes, update both the template manifest and this table, then keep the workflow
wrappers untouched.
...
| E2E (deterministic gate) | `cd frontend && pnpm test:e2e --project=chromium` | Run against the full Docker stack. Chromium is the only pass/fail browser for the local gate by default. JUnit output should land at `frontend/test-results/junit.xml`; HTML report at `frontend/playwright-report/index.html`. |
...
`./scripts/phase-gate.sh [XX]` is the preferred helper for this reference stack. It is the helper
currently declared by the installed template manifest and an implementation of these gate commands,
...
- E2E is local-first in this template: run it through `/phase-gate` before commit.
- The default CI workflow does not run Playwright.
- If a derived project later decides to add CI E2E, use [E2E Pipeline Checklist](./E2E_PIPELINE_CHECKLIST.md) as an opt-in rollout guide.
