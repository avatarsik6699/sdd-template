...
Stand up the complete foundational infrastructure: Docker services, database with migrations, JWT authentication, and a Nuxt skeleton with auth guards. This is the bedrock on which all future phases build — nothing can proceed until these gate checks are green.
